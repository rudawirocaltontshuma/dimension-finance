...
function statusBadgeClass(status: CreditNote["status"]) {
  if (status === "Applied")
...
export function CreditNotesTable({ creditNotes }: { creditNotes: CreditNote[] }) {
  ...
  const columns = React.useMemo<ColumnDef<DataTableFeatures, CreditNote>[]>(
    () => [
      {
        id: "search",
      ...
      { accessorKey: "customerName", header: "Customer" },
      {
        ...
        header: "Invoice",
        cell: ({ row }) => <span className="font-mono text-xs">{row.original.invoiceId}</span>,
      },
      {
        accessorKey: "date",
        header: "Date",
    ...
    <>
      <Card>
        <CardHeader className="border-b has-data-[slot=card-action]:grid-cols-1 md:has-data-[slot=card-action]:grid-cols-[1fr_auto]">
          <CardTitle className="text-xl leading-none">Credit Notes</CardTitle>
          <CardAction className="col-start-1 row-start-auto flex w-full flex-wrap justify-start gap-2 justify-self-stretch md:col-start-2 md:row-span-2 md:row-start-1 md:w-auto md:flex-nowrap md:justify-end md:justify-self-end">
            <TableSearchInput table={table} placeholder="Search credit notes…" />
            <ExportPreviewButton />
          </CardAction>
        </CardHeader>
        <CardContent className="flex flex-col gap-4 px-0">
          ...
          <div className="px-4">
            <FinanceDataTable table={table} emptyMessage="No credit notes match your filters." />
          </div>
        </CardContent>
      </Card>
      ...
      <DetailDrawer
        ...
        title={selected?.id ?? ""}
        description={selected ? `Issued to ${selected.customerName}` : undefined}
      >
        {selected && (
          <div className="space-y-4 py-4 text-sm">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <p className="font-medium">{selected.invoiceId}</p>
              </div>
              <div>
