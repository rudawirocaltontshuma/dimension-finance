...
import { customerFormFields } from "@/components/finance/entity-form/entity-configs";
...
import { customerFilterOptions } from "@/data/customers";
    ...
    <Card>
      <CardHeader className="border-b has-data-[slot=card-action]:grid-cols-1 md:has-data-[slot=card-action]:grid-cols-[1fr_auto]">
        ...
        <CardAction className="col-start-1 row-start-auto flex w-full flex-wrap justify-start gap-2 justify-self-stretch md:col-start-2 md:row-span-2 md:row-start-1 md:w-auto md:flex-nowrap md:justify-end md:justify-self-end">
          <TableSearchInput table={table} placeholder="Search customers…" />
          <ExportPreviewButton />
          <NewRecordDialog
            title="New Customer"
            description="Create a demo customer record. Nothing is saved to a real database."
      ...
      <CardContent className="flex flex-col gap-4 px-0">
        <div className="flex flex-wrap items-center gap-3 px-4">
          <TableFilterSelect
            ...
            label="Credit"
            options={customerFilterOptions.creditStatus}
          />
        </div>
        <div className="px-4">
          <FinanceDataTable table={table} emptyMessage="No customers match your filters." />
        </div>
      </CardContent>
    </Card>
  );
