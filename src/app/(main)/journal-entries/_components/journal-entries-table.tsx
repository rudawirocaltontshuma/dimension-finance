...
import { journalEntryStatusOptions } from "@/data/journalEntries";
import { dataTableFeatures } from "@/lib/data-table-features";
import type { JournalEntry } from "@/types/finance";

import { journalEntriesColumns } from "./journal-entries-columns";
  ...
  return (
    <Card>
      <CardHeader className="border-b has-data-[slot=card-action]:grid-cols-1 md:has-data-[slot=card-action]:grid-cols-[1fr_auto]">
        <CardTitle className="text-xl leading-none">Journal Entries</CardTitle>
        <CardAction className="col-start-1 row-start-auto flex w-full flex-wrap justify-start gap-2 justify-self-stretch md:col-start-2 md:row-span-2 md:row-start-1 md:w-auto md:flex-nowrap md:justify-end md:justify-self-end">
          <TableSearchInput table={table} placeholder="Search journal entries…" />
        </CardAction>
      </CardHeader>
      <CardContent className="flex flex-col gap-4 px-0">
        <div className="flex flex-wrap items-center gap-3 px-4">
          <TableFilterSelect
            ...
            columnId="status"
            label="Status"
            options={["All", ...journalEntryStatusOptions]}
          />
        </div>
        <div className="px-4">
          <FinanceDataTable table={table} emptyMessage="No journal entries match your filters." />
        </div>
      </CardContent>
    </Card>
  );
